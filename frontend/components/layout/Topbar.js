...
export function Topbar({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-20 mb-2 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white/95 px-4 py-2 shadow-sm backdrop-blur-sm sm:mb-3 sm:px-6 sm:py-2.5 dark:bg-slate-900/90 dark:text-slate-100">
      ...
      <nav className="hidden min-w-0 flex-1 flex-wrap items-center justify-center gap-3 py-1 text-xs font-medium text-slate-600 lg:flex lg:justify-center dark:text-slate-200">
        {navItems.map((item) => {
          const active =
            pathname === item.href ||
              ...
              {notifLoading ? (
                ...
                <div className="space-y-1 max-h-64 overflow-y-auto">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className="rounded-xl bg-slate-50 px-3 py-2 hover:bg-slate-100"
                    >
